<div class="container-fluid">
    <h2>{{modedsc}}</h2>
    <form action="index.php?page=administrador_color&mode={{mode}}&id_color={{id_color}}" method="post"
        class="card">
        <div class="card-body">
            <div class="row g-2">
                <!--ID Color-->
                <div class="mb-3 col-2">
                    ...
                    <input type="hidden" id="id_color" name="id_color" value="{{id_color}}" />
                    <input type="hidden" id="mode" name="mode" value="{{mode}}" />
                    <input type="hidden" name="xssToken" value="{{xssToken}}" />
                    <input type="number" readonly name="id_colordummy" class="form-control" value="{{id_color}}"
                ...
                <div class="mb-3 col-6">
                    ...
                    <input type="text" {{readonly}} class="form-control" id="nombre" name="nombre" value="{{nombre}}"
                        placeholder="Nombre del color" {{readonly}}>
                </div>
                {{if nombre_error}}
                ...
                <div class="col-3">
                    <div class="border border-top-0 border-indigo">
                        <label class="input-group-text" for="color">Estado</label>
                        <div class="mt-2 form-check mb-2 col-lg-1 col-md-4">
                            ...
                            <div class="form-check">
                                <input type="radio" id="estado" name="estado" class="form-check-input" value="INA"
                                    {{estado_INA}} style="border:1px solid var(--bs-indigo);" {{disabled}}/>
...
<script>
    document.addEventListener("DOMContentLoaded", function () {
        document.getElementById("btnCancelar").addEventListener("click", function (e) {
            e.preventDefault();
            e.stopPropagation();
            window.location.assign("index.php?page=administrador_colores");
        });
    });
</script>